...
| 패키지 | 범위 | 주요 엔드포인트 |
|---|---|---|
| `auth` | 회원가입 · 로그인 · 토큰 발급 | `POST /api/v1/auth/**` |
| `user` | 프로필 · 기본 재무정보 · 투자성향 설문(`InvestmentProfile`) | `/api/v1/onboarding/**`, `/api/v1/users/**` |
| `couple` | 초대 코드(`Invitation`) · 커플 연결/해제 | `/api/v1/couples/**` |
| `goal` | 부부 공동 목표 | `/api/v1/goals/**` |
| `report` | AI 투자 추천(`InvestmentReport`) | `/api/v1/goals/me/report` (예정) |
